...
class PMRent(models.Model):
    ...
    contact_phone = fields.Char(string="Contact Phone",compute="_compute_contact_phone",store=True)
    ...
    def _compute_contact_phone(self):
        for rent in self:
            if not rent.contact_phone:
                rent.contact_phone = rent.tenant_id.phone or rent.tenant_id.mobile or False
    ...
    def _onchange_dates_for_checkout(self):
        if self.date_end:
            h_float = self.property_id.checkout_time or float(
                self.env['ir.config_parameter'].sudo().get_param('property_management.pm_checkout_time', 10.0)
            )
            ...
            tz = pytz.timezone(self.property_id.partner_id.tz or self.env.user.tz or 'UTC')
    ...
    def action_active(self):
        ...
        if not (self.date_start <= now <= self.date_end):
            raise ValidationError(_(
                ...
                fields.Datetime.context_timestamp(self, now).strftime('%Y-%m-%d %H:%M:%S'),
                fields.Datetime.context_timestamp(self, self.date_start).strftime('%Y-%m-%d %H:%M:%S'),
                fields.Datetime.context_timestamp(self, self.date_end).strftime('%Y-%m-%d %H:%M:%S')
            ))
        ...
        if active_rents:
            ...
            raise ValidationError(_(
                "La propiedad '%s' ya tiene un alquiler activo (Código: %s). "
                "Finalizá el anterior antes de activar este."
            ) % (self.property_id.name, other_rent_name))
