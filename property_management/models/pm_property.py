...
class PMProperty(models.Model):
    ...
    upcomming_rent_ids = fields.Many2many(
        "pm.rent", compute="_compute_rent_data", store=True
    )
    ...
    def _compute_rent_data(self):
        for record in self:
            ...
            record.upcomming_rent_ids = record.rent_ids.filtered(
                lambda rent: rent.state in ["active", "reserved"]
            )
            record.tenant_id = record.rent_ids.filtered(
                lambda rent: rent.state == "active"
            ).tenant_id
